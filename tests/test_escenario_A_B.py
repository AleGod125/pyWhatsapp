...
def test_el_mapeo_PN_LID_es_lo_unico_global_y_solo_rellena_nulos():
    ...
    from app.services import contacts_service, lid_bridge

    for funcion in (
        contacts_service.resolve_lids_via_usync,
        lid_bridge.sync_lid_map,
    ):
        fuente = inspect.getsource(funcion)
        # Solo se toca `lid`. Cualquier otra columna aqui seria un dato de una
        # cuenta viajando a otra.
        valores = [
            linea for linea in fuente.splitlines() if ".values(" in linea
        ]
        assert valores, funcion.__name__
        cuerpo = fuente.split(".values(", 1)[1]
        for prohibida in ("display_name", "push_name", "name=", "text"):
            assert prohibida not in cuerpo.split(")", 1)[0], (
                f"{funcion.__name__} escribe {prohibida} sin acotar por cuenta"
            )
        # Y solo rellena huecos: nunca pisa un LID ya resuelto.
        assert "lid.is_(None)" in fuente or "lid IS NULL" in fuente, funcion.__name__
