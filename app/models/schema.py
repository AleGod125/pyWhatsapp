...
HISTORY_STATUSES = (
    ...
    # --- Estados honestos para los chats que no se pueden excavar ---
    #
    # ``no_valid_cursor`` decia COMO estamos, no QUE pasa. Un chat con cero
    # mensajes que en el telefono si los tiene NO esta "sincronizado": esta
    # esperando una semilla. Y uno que de verdad esta vacio tampoco es lo
    # mismo. Se separan porque el usuario merece saber cual es cual.
    #
    # WhatsApp entrego la conversacion pero SIN ningun mensaje. No hay ancla
    # desde la que pedir historial. En cuanto llegue un mensaje real (en vivo
    # o por History Sync) ese mensaje se convierte en ancla y el chat pasa a
    # 'pending' para excavarse.
    "waiting_seed",
    # Se comprobo que la conversacion esta vacia de verdad.
    "empty_confirmed",
)

# Estados en los que el chat NO se puede excavar por falta de ancla.
SEEDLESS_STATUSES = ("no_valid_cursor", "waiting_seed")

# Estados que significan "ya no queda nada que pedir".
COMPLETE_STATUSES = ("exhausted", "empty_confirmed")
...
class ChatHistoryState(Base):
    ...
    # Ultimo intento de conseguir una referencia para este chat.
    #
    # "No se encontro" NO es un error ni un estado final: el chat sigue
    # esperando y se puede reintentar. Guardarlo permite decirle al usuario
    # cuando se probo por ultima vez, en vez de dejarle adivinando.
    last_seed_attempt_at: Mapped[datetime | None] = mapped_column(
