"""Dejar la base lista para la fase de almacenamiento en Drive, desde cero.

QUE HACE
--------
Vacia los DATOS operativos (chats, mensajes, multimedia, cursores, contactos,
cuentas, sesiones web, credenciales de Google, segmentos, trabajos de subida y
claves de contenido), conserva el esquema y aplica las migraciones pendientes.

Tambien limpia la cache local de multimedia: son copias, no originales.

POR QUE HACE FALTA
------------------
Los mensajes anteriores a esta fase tienen ``storage_status='local'``: su
contenido nunca se subio y no hay segmento al que pertenezcan. El sistema NO
los inventa ni los adjudica a nadie, asi que se quedarian ahi sin poder
subirse nunca.

QUE NO BORRA
------------
Los archivos que YA esten en Google Drive. Eso lo hace, con su propia
confirmacion, ``tools/reset_drive_test_storage.py``: borrar contenido remoto
es una decision aparte de vaciar una base de desarrollo.
...
-----------
    ...
    py tools/reset_for_drive_storage.py --aplicar  # lo hace, tras confirmar
...
TABLAS = (
    # Primero lo que apunta a otras cosas.
    "storage_jobs",
    "message_segments",
    "drive_folders",
    "media_files",
...
def cache_de_multimedia(settings, *, aplicar: bool) -> list[str]:
    """La copia local de los adjuntos.

    Es CACHE: el original esta en Drive. Se puede borrar sin perder nada
    siempre que lo de Drive siga ahi, y este script no toca Drive.
    """
    carpeta = Path(settings.media_dir)
    if not carpeta.exists():
        return ["  (no hay cache local)"]

    archivos = [r for r in carpeta.rglob("*") if r.is_file()]
    total = sum(r.stat().st_size for r in archivos)
    if not archivos:
        return ["  (la cache esta vacia)"]
    if not aplicar:
        return [f"  {len(archivos)} archivo(s), {total / (1024*1024):.1f} MB"]
    ...
    for ruta in archivos:
        try:
            ...
            pass
    return [f"  {borrados} archivo(s) borrados ({total / (1024*1024):.1f} MB)"]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepara la base para la fase de almacenamiento en Drive."
    )
    ...
    parser.add_argument(
        "--cache",
        action="store_true",
        help="borrar tambien la cache local de multimedia (data/media/)",
    )
    ...
    if args.cache:
        print("")
        print("Cache local de multimedia:")
        for linea in cache_de_multimedia(settings, aplicar=False):
            print(linea)

    print("")
    print("Google Drive: NO se toca nada aqui.")
    print("Para borrar lo ya subido, con su propia confirmacion:")
    print("    py tools/reset_drive_test_storage.py")
    ...
    if not args.aplicar:
        ...
        print("    py tools/reset_for_drive_storage.py --aplicar")
        return 0
    ...
    if args.cache:
        print("")
        print("Borrando cache local de multimedia...")
        for linea in cache_de_multimedia(settings, aplicar=True):
            print(linea)
