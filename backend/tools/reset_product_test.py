...
def copia_de_seguridad(settings, destino: Path) -> list[str]:
    ...
    # La carpeta entera: credenciales Y almacen de Signal. Copiar solo las
    # credenciales dejaria una copia que no sirve para restaurar nada.
    carpeta = settings.session_dir_baileys
    if carpeta.is_dir():
        shutil.copytree(carpeta, destino / carpeta.name, dirs_exist_ok=True)
        hecho.append(f"sesion: {carpeta.name}/ ({len(list(carpeta.iterdir()))} ficheros)")
...
def borrar_sesion(settings, *, aplicar: bool) -> list[str]:
    """Quita la carpeta de sesion COMO UNIDAD, o no quita nada.

    Credenciales y almacen de Signal viven juntos y se van juntos. Llevarse
    medio deja un dispositivo NUEVO usando ratchets VIEJOS: el sintoma es
    ``unknown one-time pre-key id`` en cada mensaje entrante, y peticiones de
    historial que reciben ACK y despues nada.
    """
    carpeta = settings.session_dir_baileys
    if not carpeta.is_dir() or not any(carpeta.iterdir()):
        return ["  (no hay sesion principal)"]

    dentro = sorted(carpeta.iterdir())
    lineas = [f"  {carpeta.name}/ ({len(dentro)} ficheros)"]
    if not aplicar:
        return lineas
    ...
    try:
        ...
        lineas.append(f"  !! NO se pudo borrar la carpeta entera: {exc}")
        lineas.append("  !! NO vincules todavia: quedaria una identidad mezclada.")
        lineas.append("  !! Para el servicio y vuelve a ejecutar esto.")
        return lineas

    # Y se comprueba, en vez de confiar en que rmtree dijo la verdad.
    if carpeta.exists() and any(carpeta.iterdir()):
        restos = [r.name for r in carpeta.iterdir()]
        lineas.append("  !! SIGUEN AHI: " + ", ".join(restos))
    else:
        lineas.append("  identidad y almacen de Signal borrados juntos")
    return lineas
