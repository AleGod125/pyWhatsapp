...
def test_una_desconocida_se_acepta_si_soy_la_unica_cuenta():
    """Con una sola cuenta no hay ninguna otra a la que pudiera pertenecer."""
    assert _es_mia("nueva@s.whatsapp.net", "yo", {}, sola=True) is True
...
# ---------------------------------------------------------------------------
# Que el ciclo lo llame, y solo en el modo profundo
# ---------------------------------------------------------------------------


def test_la_fase_archivo_solo_corre_en_la_revision_completa():
...
def test_el_archivo_se_relee_ANTES_de_pedirle_nada_al_telefono():
    """Pedir lo que ya esta en casa gasta la unica ranura de peticiones."""
    import inspect
    ...
    fuente = inspect.getsource(SyncJob._ciclo)
    assert fuente.index("_fase_archivo") < fuente.index("_fase_backfill")


def test_la_fase_se_acota_a_la_cuenta_que_pulsa():
    import inspect
