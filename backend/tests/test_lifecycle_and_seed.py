...
def test_un_401_no_destruye_la_sesion(runtime, cuenta):
    ...
    runtime.pairing._on_renew = lambda: None
    from tests.conftest import escribir_credenciales

    escribir_credenciales(runtime.settings)
...
def test_archivar_no_lanza_aunque_haya_archivos_bloqueados(settings, tmp_path):
    ...
    # La carpeta de Baileys es la UNIDAD que se archiva: credenciales y
    # almacen de Signal viven juntos y se mueven juntos.
    carpeta = sesion / "baileys"
    carpeta.mkdir(parents=True)
    (carpeta / "creds.json").write_text("{}", encoding="utf-8")
    (carpeta / "session-573000.json").write_text("x", encoding="utf-8")
    ...
    with open(carpeta / "session-573000.json", "r", encoding="utf-8"):
        destino = archive_session(aislado, reason="prueba")

    # Las credenciales tienen que salir de en medio pase lo que pase.
    assert not (carpeta / "creds.json").exists()
