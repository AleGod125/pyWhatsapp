...
def test_la_forma_de_la_peticion_es_la_de_siempre(servicio):
    ...
    # El tamano del trozo sale de la configuracion, no de un numero escrito
    # aqui: se subio a 500 para la extraccion profunda y esta prueba fallaba
    # sin que nada estuviera roto.
    from app.core.config import load_settings

    assert peticion.onDemandMsgCount == load_settings().history_on_demand_count
...
def test_una_respuesta_real_deja_todo_lo_que_hay_que_medir(servicio_paciente):
    ...
    # `messages` es lo que LLEGO (la respuesta simulada trae 50); `count` es
    # lo que se PIDIO, y eso si sale de la configuracion.
    esperado = load_settings().history_on_demand_count
