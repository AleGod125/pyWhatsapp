...
def runtime(settings, tmp_path):
    ...
    escribir_credenciales(aislado, pn="573002389304", dispositivo=84)
    return rt
...
def test_la_sesion_se_archiva_no_se_borra(runtime):
    ...
    assert any((d / "creds.json").exists() for d in archivadas)
...
def test_si_no_se_puede_apartar_la_sesion_se_dice(runtime, monkeypatch):
    ...
    # El archivado no se lleva nada: las credenciales siguen ahi.
...
def con_cliente_falso(runtime, monkeypatch):
    ...
    # El runtime construye el cliente por la FABRICA, no por la clase: asi la
    # prueba no depende de que proveedor haya detras.
    monkeypatch.setattr(
        wa_module, "crear_cliente_de_whatsapp", lambda *a, **k: _ClienteFalso()
    )
