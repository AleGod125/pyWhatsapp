...
def test_el_env_de_ejemplo_documenta_el_riesgo():
    # Ruta relativa NO: el backend ya no corre desde la raiz del
    # repositorio, y `.env.example` vive alli, junto al `.env` que
    # comparten backend y frontend.
    from app.core.config import buscar_env

    plantilla = buscar_env(nombre=".env.example")
    assert plantilla is not None, "no se encuentra .env.example"
    ejemplo = plantilla.read_text(encoding="utf-8")
