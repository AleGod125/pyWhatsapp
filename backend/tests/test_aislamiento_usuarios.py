...
def test_el_listado_solo_trae_los_chats_propios(app_de_pruebas, runtime, dos_usuarios):
    ...
    # `todos=1`: los chats de la fixture no tienen mensajes y el listado los
    # oculta por defecto. Lo que se prueba aqui es el AISLAMIENTO, no el
    # filtro, asi que se pide la lista completa.
    jids = {
        c["jid"]
        for c in cliente_a.get("/api/v1/chats?todos=1").get_json()["chats"]
    }
