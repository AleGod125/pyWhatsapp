...
Se construyen eventos con la MISMA forma que emite el proveedor
(``MensajeEnVivo``, el objeto real que llega por el sink, no un doble) y se
comprueba que acaban en PostgreSQL sin duplicarse ni pisar lo que ya hubiera.

EL ADJUNTO ES OTRA HISTORIA
---------------------------
Con pywhats el adjunto viajaba en el propio evento, como ``MediaAttachment``.
Con Baileys ``media`` llega SIEMPRE a ``None`` a proposito: el adjunto lo
detecta nuestro parser desde ``raw_proto``, que es el mismo camino que ya
usaban los mensajes salientes. Un camino en vez de dos.

Aqui se sigue probando la clasificacion --que un ``ptt`` sea nota de voz y no
audio-- con un objeto que expone lo unico que se le mira: ``kind`` y ``ptt``.
"""

from __future__ import annotations

import pytest
from app.wa.baileys_client import MensajeEnVivo as Message
...
class _Adjunto:
    """Lo que el pipeline le mira de verdad a un adjunto.

    ``message_type_for`` lee ``kind`` y ``ptt``; el registro de multimedia lee
    el resto. No es un doble de conveniencia: son los campos que existen.
    """
