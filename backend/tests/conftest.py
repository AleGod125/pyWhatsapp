...
def escribir_credenciales(
    ...
    *,
    ...
    """Escribe un ``creds.json`` con la forma EXACTA que guarda Baileys.

    Antes las pruebas escribian a mano el ``device.json`` de pywhats, que
    tenia otra forma (``{"jid": {"user": ...}}``) y vivia un nivel mas
    arriba. Se centraliza aqui para que la forma real este en un solo sitio:
    si Baileys cambia como persiste la identidad, se toca una funcion.

    El JID propio se guarda CON la ranura de dispositivo
    (``573...:1@s.whatsapp.net``), igual que hace Baileys; quitarla es
    responsabilidad de quien lo lee.
    """
    import json

    ruta = settings.session_file
    ruta.parent.mkdir(parents=True, exist_ok=True)
    yo = {"id": f"{pn}:{dispositivo}@s.whatsapp.net", "name": nombre}
    if lid:
        yo["lid"] = f"{lid}:{dispositivo}@lid"
    ruta.write_text(
        json.dumps({"me": yo, "registrationId": registration_id}), encoding="utf-8"
    )
    return ruta
