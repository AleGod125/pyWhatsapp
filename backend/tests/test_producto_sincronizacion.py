...
def test_un_lid_sin_nombre_no_se_ensena_como_nombre():
    ...
    Y se distingue de "no tiene nombre guardado": aqui no sabemos ni su
    numero, porque el par LID<->PN todavia no ha llegado. Decirlo separa "esta
    persona no esta en tu agenda" de "falta un dato que si va a llegar".
    """
    from app.services.repository import display_name_for

    nombre = display_name_for("21935119425699@lid")
    assert nombre == "Desconocido (lid)"
    assert "21935119425699" not in nombre, "el identificador interno no se ensena"
