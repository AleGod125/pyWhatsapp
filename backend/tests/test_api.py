...
def test_ningun_modulo_vuelve_a_importar_tkinter():
    ...
    """
    for paquete in ("app.api", "app.core", "app.services", "app.wa"):
...
def test_un_chat_sin_mensajes_se_oculta_PERO_se_cuenta(cliente, session, cuenta_del_cliente):
    """Ocultarlo sin decirlo seria mentir por omision. Se dice.

    De 205 conversaciones traidas, 164 no tenian NI UN mensaje escrito: son
    contactos a los que WhatsApp reparte un aviso de cifrado y conversaciones
    que aun no se han excavado. Llenaban el panel y tapaban las 41 reales.

    Asi que se ocultan por defecto, pero el listado dice cuantas hay en total
    y `?todos=1` las trae. Lo que no puede pasar es que desaparezcan sin
    dejar rastro: entonces no se distingue de una extraccion que fallo.
    """
    ...
    assert jid not in {c["jid"] for c in cuerpo["chats"]}
    assert cuerpo["sin_mensajes"] >= 1, "se oculto sin contarlo"
    assert cuerpo["total_conversaciones"] > cuerpo["count"]

    todas = cliente.get("/api/v1/chats?limit=1000&todos=1").get_json()
    assert jid in {c["jid"] for c in todas["chats"]}


def test_la_cuenta_de_servicio_de_whatsapp_nunca_se_lista(cliente, session, cuenta_del_cliente):
    """`0@s.whatsapp.net` se pintaba como un contacto llamado "+0".

    No es una persona: es por donde entran los avisos del propio sistema.
    """
    from app.models import Chat
    ...
    for url in ("/api/v1/chats?limit=1000", "/api/v1/chats?limit=1000&todos=1"):
        cuerpo = cliente.get(url).get_json()
        assert "0@s.whatsapp.net" not in {c["jid"] for c in cuerpo["chats"]}, url
