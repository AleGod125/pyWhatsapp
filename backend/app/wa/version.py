"""Que version de WhatsApp Web anunciar, resuelta EN VIVO.

POR QUE SIGUE AQUI
------------------
Este resolutor nacio para rodear un bug de pywhats, pero resulto ser mejor que
lo que trae Baileys y por eso se conserva: ``fetchLatestBaileysVersion()``
devolvio 2.3000.1043857760 mientras esto encontraba 2.3000.1047094411 en vivo
--tres millones de revisiones mas nueva--.

DE DONDE SALE
-------------
Del ``sw.js`` de web.whatsapp.com, que lleva ``client_revision`` dentro de un
blob JSON incrustado. Las cabeceras de ``_HEADERS`` no son decorativas: sin
``Sec-Fetch-Dest`` ni ``Service-Worker`` el servidor contesta 400.

QUE NO HACE YA
--------------
Inyectar la version en nadie. Antes reasignaba ``WA_WEB_VERSION`` en DOS
espacios de nombres de pywhats, porque ``pairing`` la habia importado por
copia. Ahora simplemente se resuelve y se le pasa al worker por
``WA_BAILEYS_VERSION``.

Y CONVIENE SABER QUE NO ES CRITICO
----------------------------------
Se midio al aislar un fallo de vinculacion: con la version compilada en
Baileys y con la de ``sw.js`` en vivo, el servidor se comporta IGUAL. Lo que
decide que la vinculacion funcione es el perfil de navegador, no esto. Asi
que si la resolucion falla, no se para nada: se sigue con la de la libreria.
"""
