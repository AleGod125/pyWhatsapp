...
def es_chat_propio(jid: Any, propios: "tuple[str | None, str | None] | None") -> bool:
    """Si esta conversacion es la del usuario CONSIGO MISMO.

    WhatsApp la trata como una conversacion mas, identificada por el propio
    numero (o por el propio LID). Se compara solo la parte de usuario: el
    servidor puede entregarla por cualquiera de las dos direcciones, y son
    espacios distintos que no se convierten el uno en el otro.
    """
    if not jid or not propios:
        return False
    usuario = str(jid).split("@")[0].split(":")[0].split(".")[0]
    for propio in propios:
        if not propio:
            continue
        if usuario == str(propio).split("@")[0].split(":")[0].split(".")[0]:
            return True
    return False


def chat_to_json(
    summary: Any, propios: "tuple[str | None, str | None] | None" = None
) -> dict[str, Any]:
    """Fila del sidebar.

    ``propios`` es el par (PN, LID) de quien esta mirando, para poder marcar
    su propia conversacion. Es OPCIONAL a proposito: quien no lo tenga a mano
    sigue serializando igual, y el chat simplemente no se marca. Se prefiere
    eso a que un serializador tenga que ir a buscar la identidad por su
    cuenta.
    """
    ...
    return {
        "id": summary.id,
        "jid": summary.jid,
        # El chat de "mensajes contigo mismo". Se MARCA, no se oculta: quien
        # decide si mostrarlo es la pantalla. Los mensajes siguen guardados y
        # subidos a Drive igual que los de cualquier otra conversacion.
        "self_chat": es_chat_propio(summary.jid, propios),
