...
``app.wa.historial``. Aqui solo se emite la peticion y se espera a que
...
def build_on_demand_message(
    *,
    ...
    """
    from app.wa.proto import Message as PyWhatsMessage
...
class BackfillService:
    ...
    def __init__(
        ...
        *,
        ...
        # Aviso de avance, una vez por conversacion terminada.
        #
        # Sin esto el progreso EXISTIA pero no salia del proceso: se escribia
        # al log (`[HISTORY] n/total`) y nada mas, asi que la barra del panel
        # se quedaba en "0 / 205 chats · 0 mensajes" durante toda la
        # excavacion y solo saltaba al final. Parecia colgada sin estarlo.
    ...
    async def _procesar_en_paralelo(
        ...
        async def _trabajador() -> None:
            while True:
                ...
                # UNA LINEA POR CONVERSACION TERMINADA, con el avance global.
                #
                # Antes solo se veia "agotado" por chat y un resumen al final:
                # en una extraccion profunda de decenas de conversaciones no
                # habia forma de saber si avanzaba o se habia quedado colgada.
                log.info(
                    "[HISTORY] %d/%d conversaciones · %d mensajes nuevos · "
                    "%d peticiones · %d timeouts",
                    self.stats.chats_processed,
                    total_pendientes,
                    self.stats.messages_new,
                    self.stats.requests_sent,
                    self.stats.timeouts,
                )
                # Y lo mismo hacia la pantalla. Nunca lanza: un aviso que
                # falla no puede cortar una excavacion a medias.
                if self.on_progress is not None:
                    try:
                        self.on_progress(
                            self.stats.chats_processed, total_pendientes
                        )
                    except Exception:  # noqa: BLE001
                        log.debug("No se pudo publicar el avance del backfill")

        total_pendientes = len(pendientes)
        log.info(
            "[HISTORY] excavacion: %d conversaciones, hasta %d a la vez, "
            "trozos de %d mensajes",
            total_pendientes,
            trabajadores,
    ...
    async def _diagnostico_locked(
        ...
        try:
            ...
            # La stanza sale ya con category="peer": el worker la manda con
            # `sendPeerDataOperationMessage`. Con pywhats habia que forzarlo
            # parcheando el emisor, y sin ese atributo el servidor confirmaba
            # la stanza y la descartaba en silencio -- 8 peticiones, 6
            # timeouts, 0 mensajes.
            ...
            # `enc_type` lo sabia pywhats porque el parche miraba el sobre
            # Signal por dentro. Baileys cifra la stanza el mismo y no lo
            # cuenta, asi que aqui la respuesta honesta es "no se sabe".
            self.ultimo_enc_type = None
    ...
    def peer_session_state(self) -> dict[str, Any]:
        ...
        Baileys guarda cada sesion como un fichero ``session-<usuario>.<n>.json``
        dentro de la carpeta de credenciales, asi que mirar si existe es
        listar la carpeta. Antes era una consulta al SQLite de pywhats.
        ...
        try:
            ...
            carpeta = self._settings.session_dir_baileys
            nombres = [
                ruta.name for ruta in carpeta.glob("session-*.json")
            ] if carpeta.is_dir() else []
            estado["pn"] = any(usuario_pn in n for n in nombres)
            if usuario_lid:
                estado["lid"] = any(usuario_lid in n for n in nombres)
