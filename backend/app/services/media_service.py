...
class MediaStats:
    ...
    expired: int = 0
    deduplicated: int = 0
    # Adjuntos que se han metido en la cola desde que arranco el proceso.
    #
    # Hace falta porque la cola CRECE: el backfill va descubriendo multimedia
    # mientras el worker baja lo anterior. Antes se comparaba un ``processed``
    # acumulado contra el tamano del lote ACTUAL, y salian lineas imposibles
    # como "Procesando multimedia 535/387". Contando tambien los descubiertos,
    # el denominador crece con el numerador y nunca se queda corto.
    discovered: int = 0

    @property
    def remaining(self) -> int:
        """Lo que queda por procesar. Nunca negativo."""
        return max(0, self.discovered - self.processed)
...
class MediaService:
    ...
    def __init__(
        ...
        *,
        ...
        self._running = False
        # Aviso POR ADJUNTO, no por ronda. Sin el, la unica forma de que una
        # burbuja pendiente se convirtiera en una imagen era repintar el chat
        # entero al terminar toda la cola; ahora se actualiza la que acaba de
        # llegar, en cuanto llega.
    ...
    async def run(self) -> MediaStats:
        ...
        try:
            ...
            # El lote entra en el total acumulado ANTES de empezar: asi el
            # denominador ya lo incluye cuando se reporte el progreso.
            self._stats.discovered += len(ids)
            log.info(
                "Descargando %d adjuntos (concurrencia=%d, pendientes=%d)",
                len(ids),
                concurrency,
                self._stats.remaining,
            )
            ...
            progreso = asyncio.create_task(
                self._report_progress(), name="media-progress"
            )
            await queue.join()
            progreso.cancel()
    ...
    async def _report_progress(self, every: float = 10.0) -> None:
        """Resumen periodico en INFO, en lugar de una linea por adjunto.

        Se comparan dos numeros ACUMULADOS, no uno acumulado contra el tamano
        del lote de turno: mientras el worker baja, el backfill sigue
        descubriendo adjuntos, y mezclar las dos escalas producia
        "535/387".
        """
        try:
            while True:
    ...
    async def _download_one(self, media_id: int) -> None:
        from app.wa.tipos import MediaInfo
    ...
    def _finish(
        ...
        self._notify_ready(media_id, "downloaded")
    ...
    def _notify_ready(self, media_id: int, status: str) -> None:
        """Avisa de que ESTE adjunto cambio de estado.

        Se resuelven aqui el mensaje y el chat a los que pertenece porque
        quien escucha necesita saber QUE burbuja repintar, y buscarlo desde
        fuera obligaria a una consulta mas por evento.

        Nunca lanza: un fallo avisando no puede deshacer una descarga buena.
        """
        if self._on_media_ready is None:
            return
        try:
            with self._database.transaction() as session:
                fila = session.execute(
                    select(MediaFile.message_id, MediaFile.chat_id).where(
        ...
        except Exception:  # noqa: BLE001 - avisar no puede romper la descarga
            log.debug("No se pudo avisar del adjunto %d", media_id)
