...
class AppRuntime:
    ...
    def iniciar_vinculacion(self, user_id: Any, account_id: Any) -> None:
        ...
        if self.pairing.committed:
            # EL CODIGO YA SE ESCANEO. Lo que falta es que el servidor acepte
            # el login, y eso no se acelera pidiendo otro QR.
            #
            # Sin esta guarda pasaba lo siguiente, medido: llegaba el
            # pair-success, el estado iba a CONNECTING, el frontend --que
            # seguia en la pantalla del codigo-- volvia a llamar a
            # /session/pair, y esto lo devolvia a PAIRING. La vinculacion
            # recien conseguida se tiraba abajo sola, en bucle, y de paso
            # martilleaba el registro del servidor.
            log.info(
                "Peticion de vinculacion ignorada: el codigo ya se escaneo "
                "(estado=%s)",
    ...
    def start(self, *, connect: bool = True) -> "AppRuntime":
        ...
        #
        # No hay nada que preparar antes. Aqui se aplicaban los parches de
        # `app/compat/`, que rodeaban carencias de pywhats; Baileys no las
        # tiene, asi que el arranque es directo.
        self.client = crear_cliente_de_whatsapp(self.settings, self.bus)
        self._wire_services()
    ...
    def _observar_evento(self, event: Any) -> None:
        ...
        if nombre == "qr" and carga:
            ...
            # Aqui se sembraba a mano nuestro par PN<->LID en el Signal Store,
            # porque pywhats no lo registraba y cada mensaje escrito desde el
            # telefono moria con "no session for peer <nuestro propio LID>".
            # Baileys mantiene su propio almacen y resuelve el LID solo, asi
            # que no hay nada que sembrar.
            self.state.set(AppState.CONNECTED, reason="sesion aceptada")
    ...
    def _descartar_sesion_revocada(self, motivo: Any) -> None:
        ...
        # mensaje entrante.
        carpeta = self.settings.session_dir_baileys
        restos = sorted(carpeta.iterdir()) if carpeta.is_dir() else []
        if restos:
            log.error(
                "No se pudo apartar la sesion revocada entera; siguen ahi: %s. "
                "NO se vincula de nuevo: unas credenciales nuevas sobre un "
                "almacen de Signal viejo producen una identidad mezclada que "
                "no descifra nada. Cierra el servicio y arrancalo con --fresh, "
                "o mueve esa carpeta a mano. PostgreSQL no se ha tocado.",
    ...
    def restart_pairing(self) -> None:
        ...
        from app.wa import crear_cliente_de_whatsapp as _crear

        # El MISMO proveedor que en el arranque: que uno se quedara en pywhats
        # mientras el otro va a Baileys solo se notaria a mitad de una
        # re-vinculacion, con la sesion ya abierta.
        self.client = _crear(self.settings, self.bus)
        self._wire_services()
    ...
    def _wire_services(self) -> None:
        ...
        # Los acuses de reintento los lleva Baileys por dentro, con el
        # contador real y el bloque <keys>. Aqui habia un parche que espiaba
        # la salida del acuse de pywhats para poder contarlos; ya no hace
        # falta espiar nada.
        ...
        # El par PN<->LID que viaja en la clave de cada mensaje. (C-23)
        #
        # Solo lo emite el proveedor de Baileys; con pywhats este sink nunca
        # se dispara y no molesta. Es la via mas barata que hay para ese dato:
        # llega solo con cada mensaje, sin gastar una peticion de red, y de el
        # depende que 56 de 57 conversaciones dejen de verse como numeros.
        def anotar_par_lid(carga: Any) -> None:
            ...
            # LA CUENTA va tambien: sin ella el par solo puede rellenar el
            # hueco de un contacto existente, y en una instalacion nueva no
            # existe ninguno. Con ella se crea la fila con los dos
            # identificadores y sin nombre, que es justo lo que hace falta
            # para que el panel deje de mostrar codigos.
            guardar_par_lid(
                self.database,
                datos.get("lid"),
                datos.get("pn"),
        ...
        self.client.sinks["lid_pair"] = anotar_par_lid
        ...
        # LA COMPUERTA SOBREVIVE AL RECABLEADO, y esto no es un detalle.
        #
        # `restart_pairing()` reconstruye el cliente y vuelve a llamar aqui.
        # Creando una compuerta nueva cada vez, la que recibia los avisos y la
        # que esperaba podian acabar siendo objetos distintos: una veia el
        # bootstrap y la otra agotaba sus 180 segundos jurando que no habia
        # llegado. Se midio exactamente eso en un arranque real::
        #
        #     20:58:14  INITIAL_BOOTSTRAP recibido; esperando a que se asiente
        #     21:01:10  No llego INITIAL_BOOTSTRAP en 180s
        #
        # Las dos lineas son de la misma ejecucion y se contradicen. Con una
        # sola instancia eso no puede pasar.
        ...
        def ingest(full: Any) -> None:
            ...
            self.bus.publish(
                "history_ingested",
                {
                    ...
                    # Los que NO existian. La pantalla los suelta uno a uno
                    # segun se extraen, en vez de quedarse quieta hasta que el
                    # usuario recarga.
                    "new_chat_jids": list(
                        getattr(resultado, "new_chat_jids", []) or []
                    ),
                },
            )

        # EL CAMINO A LA BASE. Antes esto era `history_compat.set_callback`,
        # un parche que envolvia el manejador de pywhats. Al soltar la
        # libreria ese camino desaparecio y el historial se quedaba en disco:
        # los blobs llegaban, se archivaban y no entraban NUNCA en PostgreSQL.
        # Ahora entra por el sink, igual que los mensajes en vivo.
        self.client.sinks["history_sync"] = ingest
    ...
    def _origen_del_fallo(event: Any) -> str:
        ...
        Con pywhats esto lo contestaba un parche que clasificaba por
        DISPOSITIVO espiando el receptor. Baileys no expone ese detalle, y sin
        el la unica respuesta honesta es "venia de un par". Se conserva el
        metodo porque los contadores de fallos siguen preguntando.
        """
        return "peer"
