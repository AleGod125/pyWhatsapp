...
class PairingManager:
    ...
    def start_watchdog(self) -> None:
        ...
        if self._on_renew is None:
            return
        with self._lock:
            if self._committed or self._linked:
                # Ya hubo pair-success: lo que falta es la CONEXION, no otro
                # codigo. Relanzar el vigilante aqui devolvia la pantalla a
                # "escanea esto" con la sesion ya vinculada detras, y cada
                # ronda pedia un QR nuevo contra el servidor.
                return
