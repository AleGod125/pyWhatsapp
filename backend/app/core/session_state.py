"""Estado de la sesion: unica autoridad sobre lo que la pantalla puede mostrar.

REGLA: que existan credenciales en disco y filas en PostgreSQL NO significa
que la sesion sea valida. Solo lo es cuando el servidor acepta el login.

Por que hace falta una senal explicita
-------------------------------------
``connected`` llega en cuanto se abre el socket, ANTES de saber si el servidor
acepta la sesion. Si esta revocada, la secuencia real es::

    connected                       <- optimista
    <failure> reason=401 -- login rejected
    logged_out 401

Por eso ``connected`` no puede abrir el visor: es provisional. La confirmacion
de verdad es ``session_valid``, que el proveedor emite cuando el servidor
acepta el login y no antes; el rechazo es ``logged_out``.
...
# El aviso de "login aceptado" ya no se parchea.
#
# Aqui vivia `install_success_hook()`, que envolvia
# `SessionActivator.on_success` de pywhats para enterarse del `<success>` del
# servidor -- la unica confirmacion de que la sesion vale. Al soltar la
# libreria ese gancho desaparecio y con el la transicion a CONNECTED: la
# vinculacion funcionaba, pero la pantalla se quedaba en el codigo QR.
#
# Ahora `session_valid` es un evento del contrato (`app/wa/port.py`) que emite
# el propio proveedor, asi que no depende de las tripas de nadie.
